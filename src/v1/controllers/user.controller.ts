...
export const getUsersByProjectId = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    LoggerMiddleware.debug('Getting users for project: %s', projectId);
    const users = await userService.getUsersByProject(projectId, pageIndex, pageSize, q);
    LoggerMiddleware.info('Fetched users for project: %s', projectId);
    res.json(users);
  } catch (e: any) {
    LoggerMiddleware.error('Failed to fetch users for project: %s', e?.message || e);
    res
      .status(500)
...
export const getUsers = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Fetched users');
    res.json(users);
  } catch (e: any) {
...
export const createUser = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Created user: %o', user);
    res.status(201).json(user);
  } catch (e: any) {
    LoggerMiddleware.error('User creation failed: %s', e?.message || e);
...
export const updateUser = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Updated user: %o', user);
    res.json(user);
  } catch (e: any) {
    LoggerMiddleware.error('User update failed: %s', e?.message || e);
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Deleted user: %d', id);
    res.status(204).end();
  } catch (e: any) {
    LoggerMiddleware.error('User deletion failed: %s', e?.message || e);
    res.status(404).json({ error: 'User not found', details: e?.message || String(e) });
  }
};
