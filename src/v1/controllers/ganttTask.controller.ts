...
export const getGanttTaskById = async (req: Request, res: Response) => {
  try {
    ...
    if (task) {
      LoggerMiddleware.info('Fetched gantt task by id: %s', req.params.id);
      return res.json(task);
    }
    res.status(404).json({ error: 'Not found' });
  } catch (e: any) {
    LoggerMiddleware.error('Failed to fetch gantt task by id: %s', e?.message || e);
...
export const getGanttTasksByProjectId = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Fetched gantt tasks by projectId: %s', projectId);
    res.json(tasks);
  } catch (e: any) {
    LoggerMiddleware.error('Failed to fetch gantt tasks by projectId: %s', e?.message || e);
    res.status(500).json({
...
export const getGanttTasks = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Fetched gantt tasks');
    res.json(tasks);
  } catch (e: any) {
    LoggerMiddleware.error('Failed to fetch gantt tasks: %s', e?.message || e);
    res
      .status(500)
...
export const createGanttTask = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Created gantt task: %o', task);
    res.status(201).json(task);
  } catch (e: any) {
    LoggerMiddleware.error('GanttTask creation failed: %s', e?.message || e);
...
export const updateGanttTask = async (req: Request, res: Response) => {
  try {
    ...
    if (task) {
      LoggerMiddleware.info('Updated gantt task: %o', task);
      return res.json(task);
    }
    res.status(404).json({ error: 'Not found' });
  } catch (e: any) {
    LoggerMiddleware.error('GanttTask update failed: %s', e?.message || e);
...
export const deleteGanttTask = async (req: Request, res: Response) => {
  try {
    ...
    if (ok) {
      LoggerMiddleware.info('Deleted gantt task: %s', req.params.id);
      return res.status(204).end();
    }
    res.status(404).json({ error: 'Not found' });
  } catch (e: any) {
    LoggerMiddleware.error('GanttTask deletion failed: %s', e?.message || e);
