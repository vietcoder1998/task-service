...
export const getFiles = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Fetched files');
    res.json(files);
  } catch (e: any) {
...
export const createFile = async (req: Request, res: Response) => {
  try {
    ...
    LoggerMiddleware.info('Created file: %o', file);
    res.status(201).json(file);
  } catch (e: any) {
    LoggerMiddleware.error('File creation failed: %s', e?.message || e);
...
export const updateFile = async (req: Request, res: Response) => {
  try {
    ...
    if (file) {
      LoggerMiddleware.info('Updated file: %o', file);
      return res.json(file);
    }
    res.status(404).json({ error: 'Not found' });
  } catch (e: any) {
    LoggerMiddleware.error('File update failed: %s', e?.message || e);
...
export const deleteFile = async (req: Request, res: Response) => {
  try {
    ...
    if (ok) {
      LoggerMiddleware.info('Deleted file: %s', req.params.id);
      return res.status(204).end();
    }
    res.status(404).json({ error: 'Not found' });
  } catch (e: any) {
    LoggerMiddleware.error('File deletion failed: %s', e?.message || e);
...
export const getFile = async (req: Request, res: Response) => {
  try {
    ...
    if (file) {
      LoggerMiddleware.info('Fetched file: %o', file);
      return res.json(file);
    }
    res.status(404).json({ error: 'Not found' });
  } catch (e: any) {
    LoggerMiddleware.error('Failed to fetch file: %s', e?.message || e);
